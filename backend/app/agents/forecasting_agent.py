...
{
  "current_burn_rate": float, "runway_months": float, "runway_days": int, "currency": str,
...
BUDGET_PROMPT = """Forecast budget by category. Return ONLY valid JSON with no markdown. Ensure all amounts are in the requested currency:
{
  "monthly_forecasts": [{"month": str, "categories": [{"category": str, "projected": float}]}],
  "total_projected_3m": float, "currency": str, "growth_categories": [str], "budget_alerts": [str], "recommendations": [str]
}"""
...
class ForecastingAgent:
    ...
    async def forecast_runway(self, monthly_expenses: list[dict], cash_on_hand: float = 0, currency: str = "USD") -> dict:
        async with AgentTimer(self.AGENT_ID, self.AGENT_NAME, "forecast_runway") as timer:
            timer.input_summary = f"{len(monthly_expenses)} months, {currency} {cash_on_hand:,.0f} cash"
            if not monthly_expenses:
                return self._empty_runway(currency)
            try:
                result = await model_router.complete_with_usage(
                    task=ModelTask.FORECAST,
                    system_prompt=RUNWAY_PROMPT + f"\nNote: All values must be in {currency}.",
                    user_prompt=f"Monthly data ({currency}): {json.dumps({'expenses': monthly_expenses, 'cash': cash_on_hand}, default=str)[:2500]}",
    ...
    async def forecast_budget(self, historical_by_category: list[dict], months: int = 3, currency: str = "USD") -> dict:
        async with AgentTimer(self.AGENT_ID, self.AGENT_NAME, "forecast_budget") as timer:
            timer.input_summary = f"{len(historical_by_category)} categories, currency={currency}"
            try:
                result = await model_router.complete_with_usage(
                    task=ModelTask.FORECAST,
                    system_prompt=BUDGET_PROMPT + f"\nNote: All values must be in {currency}.",
                    user_prompt=f"Category data ({currency}): {json.dumps(historical_by_category, default=str)[:2500]}",
    ...
    def _empty_runway(self, currency: str = "USD") -> dict:
        return {"current_burn_rate": 0, "runway_months": 0, "runway_days": 0, "currency": currency,
