...
# 1. Gather all API Endpoints via regex
endpoints = []
    ...
    for file in files:
        if file.endswith('.py') and file != '__init__.py':
            ...
            for m in matches:
                ...
                # Try to extract function name right after decorator
                func_name = "unknown"
                func_match = re.search(r'def\s+([a-zA-Z0-9_]+)\(', content[m.end():])
                if func_match:
            ...
            if file.endswith(('.ts', '.tsx', '.js', '.jsx')):
                ...
                try:
                    with open(filepath, 'r', encoding='utf-8') as f:
                        lines = f.readlines()
                        for line in lines:
                            if 'apiFetch' in line or '/api/backend/' in line or 'fetch(' in line:
                                frontend_fetches.add(line.strip())
                except: pass

unused = []
for ep in endpoints:
    # Build regex matcher for frontend lines
